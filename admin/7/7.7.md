...
**Note:** The documentation of the `goobi_config.properties` is currently being revised. Due to the high demand we have published an intermediate status for the time being.

The following contents are still in progress:

- A new example configuration
- All settings concerning the METS editor
...
| `folder_processlog_internal` | Text | `internal` | Process log files are stored in this directory. |
...
| Property | Type | Default value | Description |
| -------- | ---- | ------------- | ----------- |
| `ProcessCreationResetLog` | Boolean | `false` | This value can be set to `true` to not copy the process log when duplicating a process. |
...
| `ProcessLogShowSecondField` | Boolean | `false` | This setting can be set to `true` to show a second text entry field `Version` in the process log. |
| `ProcessLogShowThirdField` | Boolean | `false` | This setting can be set to `true` to display a third text entry field `Addition` in the process log. |
...
| Property | Type | Default value | Description |
| -------- | ---- | ------------- | ----------- |
| `http_useProxy` | Boolean | `false` | This switch is set to `true` to enable the use of a proxy server. |
