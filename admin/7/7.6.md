...
| `isdoctype` | The `isdoctype` attribute can be used to control for which publication types a metadatum is to be displayed. The publication type names used here must have been defined in the `goobi_opac.xml` file. |
| `isnotdoctype` | Publikationstyp | The `isnotdoctype` attribute can be used to control for which publication types a metadate should not be displayed. This attribute can be used if an input field matches all but one specific document type. The publication type names used here must have been defined in the `goobi_opac.xml` file. |
...
The `<hide>` element can be used to hide input fields, i.e. `<item>` elements defined here. The field can contain for example the following values: `collections`, `doctype`, `preferences`, `images` and `fileUpload`.
