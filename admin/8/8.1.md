# 8.1. Installation guide - Ubuntu 20.04
...
We install pwgen to generate passwords automatically:
...
Goobi workflow is based on Java, MariaDB, Tomcat and uses Samba and OpenLdap. Maven is used as build automation tool, so some packages and dependencies have to be installed. First we set some pre-configurartion:
...
Some plugins have to be compiled and moved to the expected location in the file system for commissioning:
