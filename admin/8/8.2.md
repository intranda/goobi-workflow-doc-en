...
In addition, the `SID` for Goobi workflow is changed, the value for `userDN` is adjusted, we use the just installed ldap and remove an unused user:
...
mysql goobi -e "update ldapgruppen set authenticationType='ldap' where ldapgruppenID=2;"
