...
By updating the plugin, a `type=""` must be set for each \<rule\>. The type of existing rules is always `filter`.
For this, `type="filter"` must be set in each \<rule\> tag in the config file plugin\_intranda\_administration\_data\_poller.xml.
