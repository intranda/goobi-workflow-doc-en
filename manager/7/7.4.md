...
The GoobiScript `metadataChangeType` changes the type of a metadata. The call requires three parameters. The `oldMetadata` parameter specifies what the old type should be. The parameter `newMetadata` specifies the new type. The parameter `position` allows to specify where the metadata should be present and adapted:
